...
bool CardSprite::init(CardSuitType suit, CardFaceType face)
{
    ...
    m_clickCallback = [](CardSprite* clickedCard) {
        clickedCard->runAction(MoveTo::create(0.5f, Vec2(600, 200)));
    };
...
void CardSprite::release() {
    // 打印当前引用计数和释放前的调用位置
    CCLOG("CardSprite %p 即将释放，当前引用计数: %d",
        this, this->getReferenceCount() - 1); // 减1是因为即将执行release
    CCLOG("释放调用位置: %s (文件: %s, 行号: %d)",
        __FUNCTION__, __FILE__, __LINE__);

    // 执行父类的release逻辑
    cocos2d::Sprite::release();

    // 如果引用计数变为0，说明对象即将被销毁
    if (this->getReferenceCount() == 0) {
        CCLOG("CardSprite %p 已销毁", this);
    }
}

//get与set函数
CardSuitType CardSprite::getSuit() { return m_suit; }
CardFaceType CardSprite::getFace() { return m_face; }

bool CardSprite::getIsPlayfield() const { return _isPlayfield; }
void CardSprite::setIsPlayfield(bool value) { _isPlayfield = value; };
// isOpen 的 get/set 
bool CardSprite::getIsOpen() const { return _isOpen; }
void CardSprite::setIsOpen(bool value) { _isOpen = value; };
// position 的 get/set 
Vec2 CardSprite::getPositionInScene() const { return _positionInScene; }
void CardSprite::setPositionInScene(Vec2 value) { _positionInScene = value; };
// presentationOrder 的 get/set 
int CardSprite::getPresentationOrder() const { return _presentationOrder; }
void CardSprite::setPresentationOrder(int value) { _presentationOrder = value; };

// positionInScene 的 get/set 
Vec2 CardSprite::getOriginPosition() { return _originPosition; }
void CardSprite::setOriginPosition(Vec2 value) { _originPosition = value; };

// originPresentationOrder 的 get/set 
int CardSprite::getOriginPresentationOrder() { return _originPresentationOrder; }
void CardSprite::setOriginPresentationOrder(int value) { _originPresentationOrder = value; };
